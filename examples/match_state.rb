...
class MatchState < BasicGameState
  ID = 9 # Unique ID for this Slick game state
  ...
  def render(container, game, g)
    ...
    g.draw_string("Matching (p to pause, r to randomize, ESC to exit)", 8, container.height - 30)
  ...
  def randomize_target
    @quarry = Bug.new(rand(0..MAX_X), rand(0..MAX_Y), rand(360), rand(40..150), 0, 0, 0,0)
  end
end
