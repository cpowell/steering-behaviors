...
class FleeState < BasicGameState
  ...
  def init(container, game)
    ...
    @bug = Bug.new(MAX_X/8, MAX_Y/8, 135, 100, 0.1, 10, 0.7854, 50, 250)
  ...
  def update(container, game, delta)
    ...
    steering_force = @bug.flee(@target)
    ...
    if @bug.position_vec.x < -10 || @bug.position_vec.x > MAX_X+10 ||
      @bug.position_vec.y < -10 || @bug.position_vec.y > MAX_Y+10
      @bug.position_vec.x = rand(MAX_X/4..(3*MAX_X/4))
      @bug.position_vec.y = rand(MAX_Y/4..(3*MAX_Y/4))
      @bug.course = rand(0..360)
